...
const initLoginButton = (props: IProps) => {
  ...
  const { clientId, callbackUrl, onSuccess} = props;
  ...
  const initLoop = setInterval(() => {
    ...
    naverLogin.getLoginStatus(status => {
      ...
      console.log('access Token', naverLogin.user)
      window.opener.naver.success(naverLogin.user);
      window.close();
    })
    tryCount++;
  }, 100);
...
class LoginNaver extends React.Component<IProps, IState> {
   componentDidMount() {
    ...
    const initLoop = setInterval(() => {
      ...
      if('naver' in window) {
        window.naver.success = (data) => this.props.onSuccess(data);
